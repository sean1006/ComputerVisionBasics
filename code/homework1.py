...
if __name__ == "__main__":
    ...
    knn_runtimes = []
    features = ['sift'] * 4 + ['surf'] * 4 + ['orb'] * 4 # Order in which features were used 
    algos = ['kmeans','kmeans','hierarchical','hierarchical'] * 3
    dict_sizes = [20,50] * 6
    # Your code below, eg:
    # for i, vocab in enumerate(vocabularies):
    # ...
    ...
    for index in range(12):
        histogram_train = np.load(SAVEPATH + 'bow_train_' + str(index) + '.npy', mmap_mode='r')
        histogram_test = np.load(SAVEPATH + 'bow_test_' + str(index) + '.npy', mmap_mode='r')
        for i in ([1,3,6,9]):
            start_timer = timer()
            predicted_labels = KNN_classifier(histogram_train, train_labels, histogram_test, i)
    ...
    for index in range(12):
        ...
        histogram_train = np.load(SAVEPATH + 'bow_train_' + str(index) + '.npy', mmap_mode='r')
        histogram_test = np.load(SAVEPATH + 'bow_test_' + str(index) + '.npy', mmap_mode='r')
        predicted_labels = SVM_classifier(histogram_train, train_labels, histogram_test, True, None)
        accuracy = reportAccuracy(test_labels, predicted_labels)
        end_timer = timer()
    ...
    for index in range(12):
        ...
        histogram_train = np.load(SAVEPATH + 'bow_train_' + str(index) + '.npy', mmap_mode='r')
        histogram_test = np.load(SAVEPATH + 'bow_test_' + str(index) + '.npy', mmap_mode='r')
        predicted_labels = SVM_classifier(histogram_train, train_labels, histogram_test, False, None)
        accuracy = reportAccuracy(test_labels, predicted_labels)
        end_timer = timer()
